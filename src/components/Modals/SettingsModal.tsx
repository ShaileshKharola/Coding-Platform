...
const SettingsModal: React.FC<SettingsModalProps> = ({ setSettings, settings }) => {
	const [fontSize, setFontSize] = useLocalStorage("cwm-fontSize", "16px");
			...
			<div aria-modal='true' role='dialog' className='fixed inset-0 overflow-y-auto z-modal'>
				<div className='flex min-h-screen items-center justify-center px-4'>
					...
					<div
						...
						onClick={() => setSettings({ ...settings, settingsModalOpen: false })}
					>
						<div className='fixed inset-0 bg-gray-8 opacity-60'></div>
					</div>

					<div className='my-8 inline-block min-w-full transform rounded-[13px] text-left transition-all bg-gray-300 md:min-w-[420px] shadow-level4 shadow-lg p-0 bg-gray-300 w-[600px] !overflow-visible opacity-100 scale-100'>
						{/* setting header */}
								...
								<div className='w-[170px]'>
									<div className='relative'>
										<button
											...
											className='flex cursor-pointer items-center rounded px-3 py-1.5 text-left focus:outline-none whitespace-nowrap bg bg-gray-200 hover:bg-gray-100 active:bg-gray-100 w-full justify-between'
											type='button'
										>
											{fontSize}
											<BsChevronDown />
										...
										{settings.dropdownIsOpen && (
											<ul
												className='absolute mt-1 max-h-56 overflow-auto rounded-lg p-2 z-50 focus:outline-none shadow-lg   w-full bg-gray-200'
												style={{
...
const SettingsListItem: React.FC<SettingsListItemProps> = ({ fontSize, selectedOption, handleFontSizeChange }) => {
	return (
		<li className='relative flex h-8 cursor-pointer select-none py-1.5 pl-2 text-label-2 dark:text-dark-label-2 hover:bg-gray-100 rounded-lg'>
			<div
